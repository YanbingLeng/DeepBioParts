...
class Generator:
    ...
    def generate(
        ...
        if inference_params_dict is not None:
            ...
            if verbose:
                print(f"cached_generation=True, initializing inference_params...")
            # Try to initialize inference params and do a test forward pass
            # This is needed because LoRA-modified models may fail during forward even if initialization succeeds
            try:
                ...
                # Test forward pass with a small input to detect LoRA incompatibility early
                test_x = x[:, :1]  # Use first token only for test
                ...
                if verbose:
                    print("Cached generation test passed")
            except (RuntimeError, Exception) as e:
                # If initialization or test forward fails (e.g., with LoRA-modified models),
                # fall back to non-cached generation
                error_str = str(e)
                if any(keyword in error_str for keyword in ['inference_params', 'fir_state', 'zero batch', 'zero channel', 'fir_fn']):
                    print(f"Warning: LoRA-modified model detected. Disabling cached generation for compatibility.")
            ...
            # Initialize prefilled for non-cached generation
            if verbose:
                print(f"cached_generation=False, using non-cached generation")
            prefilled = False
            inference_params_dict = None  # Explicitly set to None
        ...
        # Flag to track if we should disable inference_params for the entire generation
        disable_inference_params = False

        for i in range(forced_prompt_length + num_tokens):
            if prefilled:
                post_prefill = True
            else:
                post_prefill = cached_generation and i > 0 and not disable_inference_params

            # prefill then process only the last token
            if post_prefill and not disable_inference_params:
                x = x[:, -1:]
            ...
            with torch.inference_mode():
                if disable_inference_params or inference_params_dict is None:
                    # Use model without inference_params
                    logits, _ = self.model(
                        x,
                        inference_params_dict=None,
                    )
                else:
                    # Try to use model with inference_params
                    try:
                        ...
                        # If forward pass fails with inference_params (e.g., with LoRA models),
                        # disable it for the rest of generation
                        error_str = str(e)
                        if any(keyword in error_str for keyword in ['inference_params', 'fir_state', 'zero batch', 'zero channel', 'fir_fn']):
                            print(f"Warning: LoRA model incompatible with inference_params. Disabling for rest of generation.")
                            disable_inference_params = True
                            inference_params_dict = None
                            # Retry without inference_params
                            logits, _ = self.model(
                                x,
                                inference_params_dict=None,
                            )
